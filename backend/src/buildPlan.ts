...
8. Сопоставь получившийся список со СТАНДАРТНЫМ БИЛДОМ (STRATZ) из сообщения пользователя: подтверди ядро билда. Затем вызови get_item_purchase_rate ПО ВСЕМ выбранным предметам-кандидатам (не только ситуативным) — он вернёт purchaseRate, winRate и avgPurchaseTimeMin (среднюю минуту покупки) для каждого. Если предмет покупают крайне редко (флаг rare) — убери его или замени на более частый аналог, если только теги не дают очень веской причины оставить именно его.
9. Расставь порядок покупки НЕ по общему ощущению, а по avgPurchaseTimeMin из шага 8: чем меньше значение — тем раньше предмет идёт в списке items и тем более ранняя у него фаза (avgPurchaseTimeMin ~< 15 → early, ~15-30 → core, > 30 или null без других данных → situational/late). Если у двух предметов avgPurchaseTimeMin близки — их относительный порядок реши по тегам/приоритету угрозы. Для каждого предмета — одна короткая причина (механика, тег, тайминг или частота покупки), явно привязанная к нужному тегу/синергии/STRATZ.
10. Перед сохранением ОБЯЗАТЕЛЬНО проверь получившийся список на предметы, которые не стакаются друг с другом: вызови get_item_details по каждому предмету билда и посмотри поля notes/hint — там прямо указано, если бонус предмета не суммируется с другими (например у всех ботинок: «Movement speed bonuses from multiple pairs of boots do not stack» — в билде не должно быть больше одной пары ботинок; у Yasha и предметов на его основе — Sange and Yasha, Manta Style, Yasha and Kaya, Trident: «Yasha-based movement speed bonuses from multiple items do not stack» — оставляй не больше одного такого предмета). Если нашёл конфликт — убери менее приоритетный из двух предметов; если нужный тег из шага 5 из-за этого остался не закрыт, замени убранный предмет на другой без конфликта. Только после этой проверки вызови submit_build: items строго в порядке покупки, у каждого item, phase и reason.
