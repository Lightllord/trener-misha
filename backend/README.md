...
| `plan_item_build` | `buildPlan.ts`  | Delayed: background full-game ordered build subagent, keyed by position (1-5). Also the sole source of item advice — the agent checks `get_build_plan` first and only calls this when no build is saved yet, or the player explicitly asks to re-think it. Stores the structured plan in `buildStore` and delivers a `build_plan` insight |
...
| `src/gameEventQueue.ts`  | Event buffer + throttling + fallback-status generation; also fires `ask_player_position` on the phase transition into `hero_selection` if the position isn't known yet, and `excess_gold` while the player is sitting on 2000+ gold (2000 + buyback cost after 30 minutes). Each kill/death/assist independently has a `SCORE_INSTANT_CHANCE` (~45%) chance of firing an immediate `score_change_instant` insight for just that event, followed by a `SCORE_INSTANT_COOLDOWN_MS` (15s) cooldown where the coin isn't flipped again; otherwise the event is batched into a `score_change` report over a `SCORE_BUFFER_WINDOW_MS` (15s) window that extends by `SCORE_BUFFER_EXTENSION_MS` (2s) per additional event but never past 15s remaining (a lone buffered kill or assist is dropped rather than reported, since it already had its shot at an instant callout). A live-score registry keeps any still-pending score insight's KDA numbers fresh even if it sits unused for a while before delivery. Enemy key-item pickups are batched per hero for `ENEMY_ITEM_BUFFER_MS` (6s) into one `enemy_key_item` report |
...
| `src/buildPlan.ts`       | Background subagent composing a full-game ordered build by position over hero/item mechanic tags + STRATZ; finishes via a `submit_build` tool that stores a structured `BuildPlan` and delivers a `build_plan` insight. Sole source of item recommendations — the agent only invokes it when no build is saved yet or the player explicitly asks to re-think the build |
| `src/buildMarkup.ts`     | Renders a `BuildPlan` into the text the voice agent reads/edits against |
| `src/itemKnowledge.ts`   | Cached loaders for the static knowledge base (hero/item tag notes, mechanics glossary, enriched items, ability files) shared by the build-plan subagent |
